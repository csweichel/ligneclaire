...
describe("koch-curves program", () => {
  it("renders a deterministic Koch path from the checked-in default parameters", () => {
    ...
    expect(document.layers[0]?.paths[0]?.closed).toBe(true);
    expect(document.layers[0]?.paths[0]?.points.length ?? 0).toBeGreaterThan(100);
  });

  it("keeps the fitted Koch curve inside the printable content bounds", () => {
    const document = expectDeterministicProgramRender(program, defaultSet, {
      caseName: "default",
      showDebug: false,
    });
