...
export function splitPathDataSubpaths(pathData: string): string[] {
...
function sampleGeometryElement(element: SVGGeometryElement): SvgOutlinePath[] {
  if (!(element instanceof SVGPathElement)) {
    const sampled = sampleSingleGeometryElement(element);
    return sampled ? [sampled] : [];
  }

  const subpaths = splitPathDataSubpaths(element.getAttribute("d") ?? "");
...
export async function importSvgOutlineFile(
  file: File
): Promise<SvgConcentricOutlineProgramState> {
